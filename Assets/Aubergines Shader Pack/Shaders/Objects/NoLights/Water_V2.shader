Shader "Aubergine/Objects/NoLights/Water_V2" {
	Properties {
		...
		Pass {
			...
			CGPROGRAM
				...
				fixed4 frag( v2f i ) : COLOR {
					half spd = _Time.y * _Speed;
					fixed3 bump = (tex2D(_BumpTex, i.uv + spd).rgb * 2 - 1) * _Bumpiness;
					bump += (tex2D(_BumpTex, i.uv - spd).rgb * 2 - 1) * _Bumpiness;
