Shader "Custom/Skin Shader Probe" {
    Properties {
    ...
    _S3STR ("SSS Strenght", Float) = 1.5
    _ProbeShininess("Probe Spec Pow", Range(1,10) ) = 2
    _ProbeSpecStr("Probe Spec Multiply", Range(0,2) ) = 0.5
    _ProbeSpecSub("Probe Spec Subtrator", Range(0.2,1) ) = 2
    _ProbePow("Probe Pow", Range(0.7,1.5) ) = 1
    }
    ...
    SubShader{
        ...
        #pragma surface surf SkinShader fullforwardshadows novertexlights
        #pragma target 3.0
...
//            fixed3 Gloss;
        ...
        float _BumpBias, _CurvatureScale, _Fresnel,_ProbePow,_ProbeShininess,_ProbeSpecStr,_ProbeSpecSub,_S3STR;
        float4 _Color;
         
        void surf (Input IN, inout SurfaceOutputSkinShader o){
            float4 albedo = tex2D ( _MainTex, IN.uv_MainTex );
            float3 Norm = UnpackNormal ( tex2D ( _BumpMap, IN.uv_MainTex ) );
            o.Normal = Norm;
...
//            o.Albedo = albedo.rgb;
            ...
            #endif
            o.Curvature = saturate( deltaWorldNormal / deltaWorldPosition ) * _CurvatureScale;
            ...
            o.Emission = (0,0,0);
            #else
            float4 Fresnel0=(1.0 - dot( normalize( float4( IN.viewDir.x, IN.viewDir.y,IN.viewDir.z,1.0 ).xyz), normalize( Norm.xyz ))).xxxx;
            fixed3 worldN = WorldNormalVector(IN, o.Normal);
            fixed3 worldR = normalize(WorldReflectionVector(IN, o.Normal));
            fixed3 worldRb = normalize(WorldReflectionVector(IN, normalize(o.NormalBlur)));
            float3 fProbeSpecCol = ShadeSH9(float4(worldR,1.0));
            float fProbeSpecMask = pow(max(_ProbeSpecStr*(length(fProbeSpecCol)-_ProbeSpecSub),0),_ProbeShininess+Fresnel0);
            o.Albedo =albedo * (ShadeSH9(float4(worldRb,1.0))*_ProbePow); //pow(ShadeSH9(float4(worldRb,1.0)),_ProbePow);
            o.Emission = (o.Albedo * pow(ShadeSH9(float4(worldR,1.0)),2)*_ProbePow) + fProbeSpecCol.rgb * fProbeSpecMask * o.Specular.r;
            #endif
        ...
        inline fixed4 LightingSkinShader( SurfaceOutputSkinShader s, fixed3 lightDir, fixed3 viewDir, fixed atten ){
            ...
            fixed4 c;
            c.rgb = (lerp(s.Albedo * saturate(NdotL) * atten, s.Albedo * (brdf*_S3STR) * m, s.Specular.b ) * _LightColor0.rgb + (spec * m * _LightColor0.rgb) ) * 2;
